...
async def read_own_items(current_user: User = Security(get_current_active_user, scopes=["display"])):
    return [{"item_id": "Foo", "owner": current_user.username}]



@app.get("/{input}")
def predict(input: str, current_user: User = Security(get_current_active_user, scopes=["display"])):
    print("user.Scopes:",current_user.scopes)
    if current_user.scopes['display']=="all":        
